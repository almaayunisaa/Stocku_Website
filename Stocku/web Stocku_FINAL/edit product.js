...
objek
  .then((result) => {
    ...
    loadKategoriDropdown(data.namaCategory); // <-- ini panggil fungsi dropdown
...
/**
 * Mengambil semua kategori dari server dan menampilkannya di dropdown.
 */
async function loadKategoriDropdown(selectedKategori) {
    const token = localStorage.getItem('authToken');
    try {
        ...
        const hasil = await res.json();
        const kategoriDropdown = document.getElementById('namaCat');
        kategoriDropdown.innerHTML = ''; 

        hasil.categories.forEach((cat) => {
            const option = document.createElement('option');
            option.value = cat.namaCategory;
            option.textContent = cat.namaCategory;

            if (cat.namaCategory === selectedKategori) {
